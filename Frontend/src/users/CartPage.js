  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (res.ok) {
          setCart(data); // Syncs with App.js state
        }
  ...
  const updateCartQuantity = (id, change) => {
    const updated = cart.map(item => {
      if (item._id === id || item.id === id) {
        const newQty = item.qty + change;
        if (change > 0 && newQty > item.stock) {
          Swal.fire({ icon: 'warning', title: 'Stock Limit', text: `Sorry, only ${item.stock} items are available.`, confirmButtonColor: '#22c55e' });
          return item;
        }
        if (newQty < 1) return item;
        return { ...item, qty: newQty };
        ...
        <div className="row">
            ...
            <div className="table-responsive shadow-sm rounded border bg-white overflow-hidden mb-4">
              <table className="table align-middle mb-0">
                <thead className="table-dark text-center">
                  <tr><th className="text-start ps-4 py-3">Product</th><th>Price</th><th>Quantity</th><th>Subtotal</th><th>Remove</th></tr>
                </thead>
                <tbody>
                  {cart.map((item) => (
                    <tr key={item._id || item.id} className="text-center cart-row">
                      <td className="text-start ps-4 py-3">
                        <div className="d-flex align-items-center">
                          <img src={item.img} alt="" className="rounded border me-3" style={{width: "70px", height: "70px", objectFit: "cover"}}/>
                      ...
                      <td>
                        <div className="qty-pill d-inline-flex align-items-center border px-3 py-1">
                          ...
                          <button className="qty-btn" onClick={() => updateCartQuantity(item._id || item.id, 1)} disabled={item.qty >= item.stock}>+</button>
                        </div>
                      </td>
                      <td className="fw-bold text-success">₹{item.price * item.qty}</td>
                      <td><button className="btn btn-link text-danger p-0" onClick={() => removeItem(item._id || item.id)}><FaTrash /></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="col-lg-4">
            <div className="card summary-card border-0 shadow-lg p-4 sticky-top" style={{top: "100px"}}>
              ...
              <div className="d-flex justify-content-between mb-3 pb-3 border-bottom"><span>Shipping</span><span className={shippingCharge === 0 ? "text-success fw-bold" : "text-danger fw-bold"}>{shippingCharge === 0 ? "FREE" : `₹${shippingCharge}`}</span></div>
              <div className="d-flex justify-content-between h3 fw-bold text-dark mt-2"><span>Total</span><span className="text-success">₹{totalAmount.toFixed(2)}</span></div>
              <button onClick={handleProceedToCheckout} className="btn btn-success w-100 mt-4 py-3 fw-bold rounded-pill shadow">PROCEED TO CHECKOUT</button>
            </div>
          </div>
        </div>
      )}
