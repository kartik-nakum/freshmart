...
function App() {
  ...
  const [categories, setCategories] = useState(["Vegetables", "Fruits", "Organic", "Leafy Greens"]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      if (userSession && isLoggedIn) {
        ...
        try {
          const res = await fetch(`https://freshmart-25n5.onrender.com/api/cart/${email}`);
  ...
  const addToCart = async (product) => {
    if (product.stock <= 0) {
      Swal.fire({ icon: 'error', title: 'Sold Out!', text: 'Sorry, this item is currently out of stock.', confirmButtonColor: '#d33' });
      return;
    }
    ...
    setVegData(prevData => prevData.map((v) => {
    ...
    Swal.fire({ icon: 'success', title: 'Added to Cart', text: `${addedQty} ${product.name} has been added successfully!`, toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
  };
    ...
    try {
      ...
      if (res.ok) {
        setWishlist((prevWishlist) => {
          ...
          if (isExist) { return prevWishlist.filter(item => item._id !== product._id); }
          else { return [...prevWishlist, product]; }
        });
        const isExist = wishlist.find(item => item._id === product._id);
        if (!isExist) {
          Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Saved to Wishlist', showConfirmButton: false, timer: 2000 });
        }
      }
    ...
    <div className="App">
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/*" element={
            <> 
              <Navbar searchTerm={searchTerm} setSearchTerm={setSearchTerm} isLoggedIn={isLoggedIn} setIsLoggedIn={setIsLoggedIn} cartCount={cart.length} wishlist={wishlist} />
              <Routes>
